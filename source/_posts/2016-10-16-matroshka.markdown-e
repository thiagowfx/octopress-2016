...
layout: post
title: "Matroshka"
date: 2016-10-16 15:49:05 -0200
comments: true
categories: ctf
---

{% img left /images/matroshka.jpg %}

Recentemente participei de uma [CTF](https://ctf.tecland.com.br/Pwn2Win/game/scoreboard/) promovida pelo [ELT](https://ctf-br.org/elt) (Epic Leet Team). Uma das *challs* que consegui resolver completamente foi a **matroshka**, e aqui está um breve *write-up* sobre a mesma.
...
De cara logo pensei em usar o [dtrx](https://brettcsmith.org/2007/dtrx/), que é um excelente programa (não perco tempo e sempre rodo um `port install dtrx`) para extrair arquivos sem ter que ficar se lembrando das sintaxes individuais de cada programa. Nesse caso, não iria rolar: os arquivos eram renomeados de forma a *trickear* o dtrx, que funciona através de heurísticas, uma delas é a 'extensão' do nome do arquivo. Por exemplo, vários arquivos (após descompactados) eram renomeados na forma `*.elt`.

A segunda alternativa foi (**serendipidade**, não conhecia essa ferramenta antes) tentar utilizar o [atool](http://www.nongnu.org/atool/). Por motivos similares ao `dtrx`, não rolou.

Pois bem, então o jeito ia ser descompactar tudo na marra. Pensei em escrever um programa que faria o seguinte:

{% codeblock lang:cpp ideia para descompactar os arquivos %}
try {
	unzip <file>
}
catch {
...
Imagem: creative commons, cortesia de *jans_world* via [Flickr](https://www.flickr.com/photos/jans_world/8985026701/in/photolist-eFYBpt-Jzabf-5vTk8Y-qkYdbS-5mzXEj-nCBUun-k5UDta-7cLP9U-vQRP4X-7Df4fG-JNWLM-BLs2ew-2jsChu-9csNyD-JzaaY-5p47z7-7JJZCz-hMnk5u-68MLQC-5p436h-6GXZ9C-h9csgL-6C6UdQ-r6dG8-yBjCHB-8MEQ8U-fSerDG-fzci3h-fyWZjg-rFJkz3-fzchZh-h8VjVf-h9dw1e-bbmHf-h9cfb2-h9cgrQ-h9cdLi-fj8d3W-h8VgK6-fiSZTT-fj2Bhz-h9cq8s-fjdWEd-fjU1ky-h9cbvg-h9cuq3-h9cqRG-for882-foFqmw-fjDRT2).